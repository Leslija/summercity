...
QBCore.Functions.CreateCallback('qb-garage-ems:CheckIfActive', function(source, cb)
    local src = source

    if not isActive then
        TriggerEvent("qb-garage-ems:server:SetActive", true)
        cb(true)
    else
        cb(false)
...
RegisterNetEvent('qb-garage-ems:server:SetActive', function(status)
    if status ~= nil then
        isActive = status
        TriggerClientEvent('qb-garage-ems:client:SetActive', -1, isActive)
    else
        TriggerClientEvent('qb-garage-ems:client:SetActive', -1, isActive)
    end
end)

RegisterServerEvent("qb-garage-ems:AddVehicleSQL")
AddEventHandler('qb-garage-ems:AddVehicleSQL', function(mods, vehicle, hash, plate)
    local src = source
...
RegisterServerEvent('qb-garage-ems:TakeMoney', function(data)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    local steamname = GetPlayerName(src)
    if Player.PlayerData.money.cash >= data.price then
        TriggerClientEvent("qb-garage-ems:SpawnVehicle", src, data.vehicle)  
