...
RegisterNetEvent('qb-garage-ems:Menu', function()
    local Menu = {
        {
            header = "EMS Garage",
            txt = "View Vehicles",
            params = {
                event = "qb-garage-ems:Catalog",
            }
        }
    }
    if not Config.UseMarkerInsteadOfMenu then
        Menu[#Menu+1] = {
            ...
            params = {
                event = "qb-garage-ems:StoreVehicle"
            }
        }
    end
    Menu[#Menu+1] = {
...
RegisterNetEvent("qb-garage-ems:Catalog", function()
    local vehicleMenu = {
        {
            ...
            if (GetDistanceBetweenCoords(plyCoords.x, plyCoords.y, plyCoords.z, 0, 0, 0, true) < 10) then
                ...
                if Config.UseMarkerInsteadOfMenu then
                    if (GetDistanceBetweenCoords(plyCoords.x, plyCoords.y, plyCoords.z, 0, 0, 0, true) < 1.5) and not IsPedInAnyVehicle(PlayerPedId(), false) then
                        DrawText3D(0, 0, 0, "~g~E~w~ - EMS Garage") 
                        if IsControlJustReleased(0, 38) then
                            TriggerEvent("qb-garage-ems:Menu")
                        end
                    end
                    ...
                    if IsControlJustReleased(0, 38) and IsPedInAnyVehicle(PlayerPedId(), false) then
                        TriggerEvent("qb-garage-ems:StoreVehicle")
                    end
                else
...
RegisterNetEvent('qb-garage-ems:client:SetActive', function(status)
    isActive = status
end)

RegisterNetEvent('qb-garage-ems:StoreVehicle')
AddEventHandler('qb-garage-ems:StoreVehicle', function()
...
RegisterNetEvent("qb-garage-ems:SpawnVehicle")
AddEventHandler("qb-garage-ems:SpawnVehicle", function(vehicle)
    local coords = vector4(329.1502, -575.6749, 28.372808, 340.50201)
...
RegisterNetEvent("qb-garage-ems:PreviewVehicle")
AddEventHandler("qb-garage-ems:PreviewVehicle", function(data)
    if Config.UsePreviewMenuSync then
        QBCore.Functions.TriggerCallback('qb-garage-ems:CheckIfActive', function(result)
            if result then
                InPreview = true
                local coords = vector4(329.1502, -575.6749, 28.372808, 340.50201)
                    ...
                    Citizen.CreateThread(function()
                        while true do
                            if IsControlJustReleased(0, 177) then
                                ...
                                InPreview = false
                                TriggerServerEvent("qb-garage-ems:server:SetActive", false)
                                break
                            end
                            Citizen.Wait(1)
                        end
