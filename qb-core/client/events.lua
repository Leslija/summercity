...
RegisterNetEvent('QBCore:Command:GoToMarker', function()
    ...
    if not DoesBlipExist(blipMarker) then
		...
		exports['mythic_notify']:DoHudText('error', 'No waypoint set', { ['background-color'] = '#FF0000', ['color'] = '#FFFFFF' })
        return 'marker'
    end
    ...
    if not found then
        ...
        SetPedCoordsKeepVehicle(ped, oldCoords['x'], oldCoords['y'], oldCoords['z'] - 1.0)
				...
				exports['mythic_notify']:DoHudText('error', 'Error While Teleporting', { ['background-color'] = '#FF0000', ['color'] = '#FFFFFF' })
    end

    -- If Z coord was found, set coords in found coords.
    SetPedCoordsKeepVehicle(ped, x, y, groundZ)
  
			exports['mythic_notify']:DoHudText('success', 'Teleported To Waypoint.', { ['background-color'] = '#2EFF00', ['color'] = '#FFFFFF' })
end)
