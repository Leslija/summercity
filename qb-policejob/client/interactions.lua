...
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        local get_Cuffed = 1;
        local success = exports['qb-lock']:StartLockPickCircle(1, get_Cuffed, success)
        StopAnimTask(ped, "mp_arrest_paired", "crook_p2_back_right", 3.0, 3.0, -1, 32, 0, 0, 0, 0 ,true, true, true);
        print(success)
        if not success then
            isHandcuffed = true
